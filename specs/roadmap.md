...
- [x] **Reorganizar `tables/` do Capítulo 4** — as tabelas importadas na rodada do capítulo empírico 1 (`specs/2026-08-29_capitulo_historico_1_review/`) tinham ficado fora do padrão `tables/capNN/` adotado depois nos capítulos 2 e 3 (2 arquivos soltos na raiz de `tables/` com prefixo `cap04-`, e 3 pastas temáticas sem prefixo de capítulo — `estatisticas_secxix/`, `divida_externa/`, `exportacoes_importacoes/`). Movidas para `tables/cap04/` (preservando as subpastas temáticas, dado o volume de arquivos), referências `\csvautotabular`/`\csvautolongtable` em `chapters/04-Processo de industrialização (1808-1973).tex` atualizadas, `main.tex` recompilado sem erros. Convenção documentada explicitamente em `specs/constitution.md`, Princípio 1.
