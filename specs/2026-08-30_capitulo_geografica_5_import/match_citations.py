...
with open(f"{BASE}\\source_raw.tex", encoding='utf-8') as f:
    full_text = unicodedata.normalize("NFC", f.read())

# restrict to body only: after abstract block (starts at "Introdução" heading),
# before the formatted reference list ("Bibliografía")
start = full_text.index("\\textbf{Introdução}")
end = full_text.index("Bibliografía")
text = full_text[start:end]

href_pattern = re.compile(r'\\href\{https://www\.zotero\.org/google-docs/\?[^}]+\}\{(\([^{}]*(?:\{[^{}]*\}[^{}]*)*\))\}')
matches = list(href_pattern.finditer(text))
print(f"Total zotero href citations found in body: {len(matches)}")
