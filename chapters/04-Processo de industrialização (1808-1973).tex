...
capital em novas atividades industriais ou auxiliares à estas. Após
...
condizente com um ``interesse comum'' \aiflag{(Robinson, ano; Amsdem ano)}. Esse
...
país \aiflag{(Santos, ano)}. Esse processo tanto servirá para constituição de
...
brasileira uma concepção de ``vocação agrícola'' do país \aiflag{(Curado, ano)}.
Isso não implica a inexistência de um pensamento industrializador ou de
associações de classe industrial, como Barreto \aiflag{(ano)} mostra no caso do
...
\aiflag{(Barreto, ano)} e um claro domínio político das elites rurais --
...
nacional classe dominante brasileira \aiflag{(Fernandes,ano)}, a proclamação da
...
\aiflag{(fonte)} já apontavam, um dos elementos importantes na compreensão da
...
da burguesia nacional numa longa transformação \aiflag{(Fernandes, ano)}.
...
\aiflag{(fonte)}, é a tradição positivista, nutrida em grande parte no meio dessa
...
processos de transformação social \aiflag{(Perissinoto)}, estas não podem ser
...
Falar mais anos 1920 \aiflag{(mello e suzigan)}
\end{esboco}
...
liberais \aiflag{(Perissinoto, ano)}. Uma conjuntura crítica irá se apresentar
...
como um período contínuo de ``desenvolvimentismo'' \aiflag{(Feijó, ano, Pereira,
Ano)}, que representaria, a parte das transformações políticas internas a
...
Mello \aiflag{(Fonte)} diverge parcialmente, enfatizando a diferenciação entre as
...
Outros autores, como Perissinoto \aiflag{(fonte)}, marcam um recorte entre 1930 e
...
análise de Barbosa \aiflag{(ano)}, que enfatiza as diferenças significativas
...
exportação tendo predomínio apenas nos anos 1940 e 1950 \aiflag{(fonte,
cambridge)}.
...
Dreyfus \aiflag{(ano)} sobre o golpe de 1964.
...
exportadora primária \aiflag{(Prado Jr)}. Particularmente o setor cafeeiro era o
...
\aiflag{(Prado Jr)}. Tal posição tanto produzia como advinha de seu
...
\aiflag{(Prado Jr)}, seria profundamente abalado com a crise de 1929. Crises
...
MAIS RAPIDO \aiflag{(MELLO)} /MEcanismo de defesa do preço do café e impulso à
indústria \aiflag{(Furtado)} / Mello JUNCAO DE MECANISMO DE DEFESA DO CAFÉ
\aiflag{\aiflag{(FURTADO)}} COM CRESCIMENTO DO CONSUMO INTERNO DEVIDO A CRESCIMENTO DA
POPULACAO, FIM DA ESCRAVIDAO E IMIGRACAO\aiflag{(Prado jr)} Desvalorizacao
cambial também gera incentivo a producao interna via queda das
importacoes \aiflag{(prado jr)} citar queima do café e convenio de taubaté $\rightarrow$
...
\aiflag{(feijó)}, ao passo que também encarece os custos da dívida contraída para
...
e internos via desvalorização cambial \aiflag{(Furtado)}, a partir de 1929 tal
...
setor cafeeiro \aiflag{(furtado)}. Assim como em períodos anteriores, conflito e
...
Suzigan\aiflag{(ano)}. Furtado também pontua como a capacidade ociosa foi central
...
\aiflag{(Perissinoto)}. Ao mesmo tempo, o tenentismo oferecia uma mistura de
positivismo, em grande parte já difundido no exército brasileiro
\aiflag{(perissinoto)} com certo progressismo modernizante. Essa composição não
era estável entretanto, com Vargas priorizando uma postura pragmatista
\aiflag{(scharwts)} e despindo o positivismo sem seus elementos mais
...
\aiflag{(Perissinoto)} deste novo governo traria para o centro da dinâmica de
...
substituição de importações, ao qual Mello \aiflag{(ano)} ainda entenderá como
...
se mostrar o mais hegemônico \aiflag{(Prado jr)} e consolidar um modelo de
...
dependente nas décadas seguintes (Evans).
...
produtivos \aiflag{(Feijó)}. Mas nossa industrialização dependeu também da
...
\aiflag{(Prado Jr)}. Além disso, Prado Jr \aiflag{(ano)} aponta a posição privilegiada do
...
privilegiada da economia brasileira \aiflag{(Prado Jr)} e estabelece relações de
...
naquilo que Evans\aiflag{(ano)} denominou uma ``Tripla Aliança'' -- setores de
...
Mas a orientação positivista do novo governo \aiflag{(perissinoto)} e dos
...
preservação do padrão ouro \aiflag{(fonte cambrige)}. Como consequência, uma
...
importações por orientação do governo \aiflag{(feijó, ano)}. Feijó et al \aiflag{(ano)}
...
acesso ao mercado nacional \aiflag{(prado jr)}. Em grande parte, esse modelo de
...
opositora de seu projeto. Perissinoto \aiflag{(fonte)} argumenta que o
...
política \aiflag{(Perissinoto)}. Gradualmente sua influência cresceu ao longo da
...
prática. Perissinotto \aiflag{(fonte)} remonta essa trajetória às ideias
...
profissionalizante \aiflag{(Perissinoto)}. Os positivistas se percebiam como
...
\aiflag{(Perissinoto)}. Em grande parte, e de forma pragmática, Vargas manteve o
...
soberania \aiflag{(fonte perissinoto)}. A atuação do governo americano, com força
...
Brasil para sua semiperiferia \aiflag{(arrighi e wallerstein)}. Se anteriormente
...
um novo modelo de acumulação do capital centrado nos EUA \aiflag{(arrighi)}.
...
em setores ``estratégicos'', como discutido por Evans \aiflag{(fonte)}.
...
Consolidação das Leis Trabalhistas, em 1943 \aiflag{(Feijó, ano)}. Na verdade,
...
acalmando temores de comunismo por parte do empresariado \aiflag{(Lilian, ano)}.
...
rural-urbano significativa, como apontado por Prado Jr \aiflag{(ano)}.
...
Barbosa \aiflag{(ano)} vai denominar como um breve período de um
...
queda. Contribuíram para isso tanto a pressão americana \aiflag{(fausto, ano)}
como as condições internas, sejam elas interpretadas como anseios
democráticos \aiflag{(schwarz)} ou a reorientação de interesses de frações de
...
entrada de dólares \aiflag{(viana e villela)}. O receituário foi portanto a
...
\aiflag{(vianna e villela)}. Evitar a desvalorização cambial não era a princípio
...
do Brasil \aiflag{(Vianna e Villela)}.
...
ao direcionamento do governo da época \aiflag{(Perissinoto ano, Bresser Pereira
ano)}. O outro elemento é a transformação das forças relativas entre as
...
trabalhista \aiflag{(vianna e villela)}. A instabilidade política voltava a se
...
processo fez prevalecer a eleição \aiflag{(Fico, ano)}.
...
mas que deu origem à Comissão Mista Brasil-EUA\aiflag{(viana e vilela)}. A
...
comerciais brasileiros \aiflag{(viana e vilela)}. Como consequência do cenário de
...
\aiflag{(tavares)}, que passa a operar o sistema de câmbio muito mais
...
novas necessidades de importação \aiflag{(tavares, ano)}, seja ligadas a nova
...
sempre presentes nos setores privado ou público \aiflag{(Tavares, ano)}. Daí a
...
processo revolucionário de 1930 \aiflag{(Perissinoto)}, mas contém ainda
significativos legados e influências liberais, talvez representadas
melhor do que ninguém pela figura de Eugênio Gudin \aiflag{(barbosa)}. Em
...
privilegiada posição social \aiflag{(barbosa)}. Em particular um conjunto de
...
criação do BNDE, o que leva Barbosa \aiflag{(fonte)} a diferenciar fortemente o
...
mas fraco desempenho do PIB e crescimento da inflação \aiflag{(viana e vilela)},
...
Weffort \aiflag{(ano)} vai classificar como um populismo, havia uma tensão entre
...
por sua vez já pressionava fortemente o governo \aiflag{(vianna e villela)}.
...
Aeronáutica \aiflag{(vianna e villetla)}. Dali ao Manifesto dos Coronéis e a
...
anti golpista resultante \aiflag{(viana e villela)}. Esse processo foi
...
mais ortodoxo da economia contemporânea brasileira \aiflag{(ano)}. O resultado é
um aumento substancial do número de falências, crise de liquidez e queda
na formação bruta de capital fixo e atividade industrial \aiflag{(Vianna e
Villela, ano)}. Novamente, seguindo o padrão, Gudin pede demissão e seu
...
Congresso, o que na prática implica o seu abandono \aiflag{(vianna e villa)},
...
aprofundamento do modelo de substituição de importações \aiflag{(tavares)}.
...
também conferia significativa vantagem ao capital estrantgeiro frente a
...
apoio suficiente para tomar posse em 1956 \aiflag{(fausto, ano; fico, ano)}. O
...
nos quatro anos seguintes \aiflag{(Villela, ano)}. Pela primeira vez existia um
...
CMBEU como do programa da CEPAL BNDE \aiflag{(abreu)}. Em 1956 é criado também o
...
\aiflag{(villela)}. Além desses, que representavam cerca de 5\% do PIB entre 1957
e 1961, haviam os recursos alocados para a construção de Brasília,
contabilmente fora desse orçamento \aiflag{(vilela)}. Dentre os setores com maior
investimento privado estiveram o setor automobilístico, a construção
naval, a mecânica pesada e o setor de equipamentos elétricos \aiflag{(vilela)}
...
oposição à carga tributária \aiflag{(Abreu)}. Extremamente, empréstimos
...
desenho claro de como financiá-lo \aiflag{(Abreu)}. Os custos nacionais foram
...
empresas. \aiflag{(ABreu)}. Já o sistema cambial, que subsidiava importação de
...
justamente a estimular o investimento industrial \aiflag{(Abreu)}. Propostas de
...
- permitindo expansão monetária irrestrita \aiflag{\aiflag{(ABREU)}}. Ao mesmo tempo, a
...
dificuldades de conter gastos com funcionalismo público \aiflag{(Abreu)}.
...
\aiflag{(Tavares)}. Assim, o sistema baseava-se em ciclos onde breves alívios das
contas externas eram seguidos de constrições na capacidade de importar,
o que particularmente se observou entre 1957 e 1959 \aiflag{(Abreu)}.
...
tornar este possível mesmo ante a falta de cobertura cambial \aiflag{(abreu)}.
...
os investimentos estrangeiros \aiflag{(villela)}. Daí a formação de um setor
...
às mesmas \aiflag{(vilella)}.

Essa espécie de ``divisão de tarefas'' é o que Evans (1979) vai entender
...
Café Filho \aiflag{(Villela)}. Junto da instrução 70, que perdurou até 1964 e
...
de câmbio para exportações sobrevalorizada \aiflag{(Villela)}. A reforma cambial
...
desenho para não encarecer as importações necessárias \aiflag{\aiflag{(ABREU)}}.
...
e do BNDE, a época presidido por Roberto Campos \aiflag{(Vilela)}. O problema foi
...
30,5\% \textbf{\aiflag{\aiflag{(VILELA)}}.} Como resultado do Plano de Metas, o PIB
...
\aiflag{(fonte)}, mas que mantinha uma organização política democrática. Assim,
...
(vilela, abreu, fausto). Seus planos são interrompidos quando Jânio opta
pela repentina renúncia e o governo Jango se torna objeto de uma crise
política.

\begin{esboco}
O período que vai de Jânio até a deposição de Jango\ldots{}

\textbf{Eleição de Janio}

subida conservadora

Projeto de austeridade

desvalorização do cambio e unificacao do mercado (204 sumor)

eliminava licensa prévia, mas exigia cobertura cambial, teve efeito
limitado na competitividade, sendo o maior impacto o fim de subsídios as
importações de petróleo, trigo e derivados. \aiflag{(ABreu)}

reescalonamento da divida com EUA; facilitado pelas medidas cambiais ,
comprometimento de divisas com o serviço da dívida externa (de 43,6\%
para 32,2\% entre 61 e 62). \aiflag{\aiflag{(ABREU)}}
\end{esboco}
...
desvalorização cambial, Jânio renuncia. \aiflag{(ABreu)}

\begin{esboco}
\textbf{Renuncia e crise em 1961}

Modelo parlamentarista até 1963
\end{esboco}

O período parlamentarista, antes da eleição de Goulart foi conturbado,
também marcado por tentativas de estabilização econômica. Havia conflito
entre Goulart, na presidencia e o gabinete Neves (com Tancredo Neves
eleito), coalizão parlamentar que liderava o país. As propostas do
governo eram excessivamente otimistas e, em meio a recrudescimento da
inflação, tentou com o programa ``Ação Emergência'' uma estratégia
contracionista. A medida foi ineficaz para reduzir deficits ou emissão
de moeda e conflitos entre governo e conglresso levaram a rejeição pelo
...
tentativas posteriores de controle inflacionario, a incapacidade da
...
mantinha uma taxa diferenciada para exportação de café, cacau e
derivados, cuja diferença para o valor de mercado era recolhida pela
SUMOC. `` (ABreu''
...
11,1\% em 1961, mas regrediu para 8,1\% em 1962. \aiflag{(Abreu)}
...
estrangeiros \aiflag{\aiflag{(ABREU)}}
...
Recessão em 1963 \aiflag{(Abreu)}  recessão industrial ''com causas que vão de
...
transporte e comunicações. \aiflag{(ABreu)}
...
Descrição geral da economia (setores, pib) - queda da agricultura

Politicas economicas

Vargas, proteção do café e impulso a indústria

Crescimento gradual da burguesia industrial

Transformação de classes e positivismo

Setores que mais se desenvolvem na década de 30

Industrialização pré e pós anos 40 (intensidade)
...
Nova constituição em 37 e a vitória em 1938

Pós 1940:Segunda guerra mundial contexto internacional, recuperação da
agricultura

Relação com EUA - nova transição da dependência (ciclos arrighi)

Setores que mais se desenvolvem

Planos econômicos e políticas econômicas

CSN, FNM e outras (estatais)

Organizações civis e industrialização

Queda de vargas (pq? O que mudou? O que não mudou?)

Pensar diferenças dos períodos: tavares e mello

Pensar similaridade: Dualidade da economia brasileira \aiflag{(Prado jr)}
